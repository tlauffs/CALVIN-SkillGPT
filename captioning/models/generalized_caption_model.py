...
class ClipGeneralizedCaptionModel(nn.Module):

    def __init__(self, prefix_length: int, clip_length: Optional[int] = None, prefix_size: int = 512,
                 num_layers: int = 8, mapping_type: MappingType = MappingType.MLP, mode: str = 'train'):
        


        super(ClipGeneralizedCaptionModel, self).__init__()
        ...
        self.mode = mode
        if self.mode == 'train': self.env_arr = ['A','B','C'] 
        else: self.env_arr = ['D']
    ...
    def forward(self, data: AttrDict):
        ...
        environment_outputs = []
        # process data for each enviornment seperatly
        for env in self.env_arr:
            actions = data.actions[env]
            observations =  data.observations[env]
            src =  AttrDict(actions = actions, observations = observations)
            behaviour_encoding  = self.behaviour_encoder(src)
            behaviour_encoder_padding_mask = ~(observations.mean(dim=2)==0.0) * 1.0
            ...
            environment_output = self.gpt(inputs_embeds=embedding_cat, labels=labels, attention_mask=total_mask)
            environment_outputs.append(environment_output)
        return environment_outputs

class ClipCaptionPrefix(ClipGeneralizedCaptionModel):
