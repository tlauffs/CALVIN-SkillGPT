...
class DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        start_episodes = {'D': 0}
        start_episodes['D'] = annotation[0][0]

        actions = {'D': torch.zeros(64, 7)}
        state = {'D': torch.zeros(64, 15)}
        observations = {'D': torch.zeros(64, 2048)}
        
        start_epi = start_episodes['D']
        for i in range(0, 63):
            epi_num = str(start_epi + i).zfill(7)
            file_path = os.path.join(self.data_path, "episode_{}.npz".format(epi_num))
            data = np.load(file_path)
            actions['D'][i] = torch.tensor(data['actions'])
            state['D'][i] = torch.tensor(data['robot_obs'])
            observations['D'][i] = torch.tensor(data['observations'])
