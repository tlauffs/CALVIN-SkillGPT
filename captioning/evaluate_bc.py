'''
    ...
    IMPORTANT: this code is broken and doesn't produce proper results
    requires calvin_env:
...
class EvaluateBCAgent():
    def __init__(self, cfg):
        ...
        self.policy.load_state_dict(torch.load(
            "/media/tim/E/hulc_captioning/captioning/checkpoints/bc_policy/bc_static_gripper_obs/bc_policy-043.pt"))
        self.policy = self.policy.to(self.device)

        self.action_stats = AttrDict(
            mean=np.array([0.05829782, -0.11443839, 0.5123094, 0.97989569, -0.03639337, 1.55281177, 0.01424668],
                          dtype=np.float32),
            std=np.array([0.1525908, 0.09533093, 0.05158806, 2.920689, 0.10144497, 0.56220544, 0.99989851]),
            dtype=np.float32)
        ...
        self.visual_autoencoder_static.load_state_dict(torch.load(
            "/media/tim/E/hulc_captioning/captioning/checkpoints/image_vae/image_vae_static_full_long/image_vae_static_full_high_res-045.pt"))
        ...
        self.visual_autoencoder_gripper.load_state_dict(torch.load(
            "/media/tim/E/hulc_captioning/captioning/checkpoints/image_vae/image_vae_gripper/image_vae_gripper_high_res-017.pt"))
